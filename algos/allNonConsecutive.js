...
function allNonConsecutive(sortedNums) {
    //declare our array to hold the dictionaries
    let results = [];
    //go through the sortedNums array
    for(let i = 0; i < sortedNums.length; i++){
        //if the next number is not one more than where we are now
        if(sortedNums[i+1] != sortedNums[i] + 1 && ()){
            results.push({i : i, n: sortedNums[i]});
        }
    }
    return results;
}
