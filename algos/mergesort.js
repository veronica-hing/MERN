...
function mergeSort(nums) {
    //use mergesort to merge sorted arrays
    //recursively divide into left and right until you get size 1 array
    //go back up the recursion chain to merge the two arrays

    //base case
    if(nums.length < 2){
        return nums;
    }
    //dividing nums into two arrays
    let left = nums.slice(0, nums.length/2);
    let right = nums.slice(nums.length/2, nums.length);
    //splice has altered nums to contain only right
    
    //call recursive splitting on the left and right sides
