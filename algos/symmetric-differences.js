...
function symmetricDifferences(numsA, numsB) {
    ...
    //to not get duplicates we use a set
    let disjoint = new Set();
    //getting items from A that aren't in B
    for(let i = 0; i < numsA.length; i++ ){
        if(!setB.has(numsA[i])){
            disjoint.add(numsA[i]);
        }
    }
    //getting items from B that aren't in A
    for(let i = 0; i < numsB.length; i++ ){
        if(!setA.has(numsB[i])){
            disjoint.add(numsB[i]);
        }
    }
    //we want to return an array, so we create one from our set
    return Array.from(disjoint);
};
